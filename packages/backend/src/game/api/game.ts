...
router.post('/create-flag', auth.tokenAuth, jsonResponse((request) => {
    ...
    .then(rooms => {
        const roomToDelete = _.findKey(rooms, flags => _.any(flags, i => i[0] == request.body.name));
        if(roomToDelete) {
    ...
    .then(() => getFlags(request.user._id))
    .then(flags => {
        const cnt = _.sum(flags, i => i.length);
...
router.post('/change-flag-color', auth.tokenAuth, jsonResponse((request, response) => {
    ...
    .then(flags => {
        const flag = _.find(flags, i => i[0] == request.body.name);
        if(flag) {
