...
export default (object, intent, {roomObjects, bulk, stats, eventLog}) => {
    ...
    const repairPower = _.filter(object.body, (i) => (i.hits > 0 || i._oldHits > 0) && i.type == C.WORK).length * C.REPAIR_POWER || 0;
    const repairEnergyRemaining = object.store.energy / C.REPAIR_COST;
    ...
    const repairCost = Math.min(object.store.energy, Math.ceil(repairEffect * C.REPAIR_COST));
