...
export default (object, intent, {roomObjects, bulk, roomController, gameTime, eventLog}) => {
    ...
    if(_.any(target.effects, e => e.effect == ScreepsConstants.EFFECT_INVULNERABILITY && e.endTime > gameTime)) {
        return;
    }

    if(target.reservation) {
        var effect = Math.floor(_.filter(object.body, (i) => i.hits > 0 && i.type == ScreepsConstants.CLAIM).length * ScreepsConstants.CONTROLLER_RESERVE);
        if(!effect) {
            return;
        }
    ...
    if(target.user) {
        var effect = Math.floor(_.filter(object.body, (i) => i.hits > 0 && i.type == ScreepsConstants.CLAIM).length * ScreepsConstants.CONTROLLER_CLAIM_DOWNGRADE);
        if(!effect) {
            return;
        }
